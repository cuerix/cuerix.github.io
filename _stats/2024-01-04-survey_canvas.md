...
기본 설문 도구
- 설명글: 응답자에게 추가 안내나 지침을 제공하는 역할을 합니다. 질문의 맥락을 설명하거나, 특정 답변 방식에 대한 가이드를 제시할 때 유용합니다. 
- 1개 선택: 여러 보기 중 하나만 선택하는 방식으로, 라디오 버튼 형태로 제공되며, 리커트 척도와 같은 평가 척도를 포함합니다.
- 다중 선택: 여러 개의 옵션을 선택할 수 있으며, 체크박스 형태로 표시됩니다.
- 수치: 숫자를 직접 입력하는 문항 유형으로, 주로 수량, 연령, 점수, 금액 등의 정량적 데이터를 수집할 때 사용됩니다.
- 텍스트: 사용자가 자유롭게 텍스트를 입력할 수 있는 형식으로, 단답형 또는 장문형으로 설정할 수 있습니다.
- 드롭다운: 카테고리를 활용해 그룹화하거나 계층 구조로 정리하여 응답자가 원하는 옵션을 쉽게 찾을 수 있도록 할 수 있습니다.
