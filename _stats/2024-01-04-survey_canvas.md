...
| 유형       | 목적                                     | 형식          | 용도                                       | 예제                                                   |
|------------|----------------------------------------|--------------|--------------------------------------------|--------------------------------------------------------|
|🔘 1개 선택  | 응답자가 한 가지 답변만 선택하도록 함  | 라디오 버튼  | 선호도, 만족도, 단일 선택이 필요한 경우   | "가장 선호하는 브랜드를 선택하세요. (A, B, C, D)"       |
|☑️ 다중 선택  | 여러 개의 답변을 선택하도록 허용      | 체크박스      | 복수 응답이 가능한 경우                   | "사용하는 SNS를 모두 선택하세요. (페이스북, 인스타, 트위터)" |
|🔢 수치       | 숫자 입력을 받음                      | 숫자 입력란  | 연령, 점수, 수량 등의 입력이 필요한 경우 | "나이를 입력하세요." / "월 평균 지출 금액을 입력하세요." |
|🔤 텍스트     | 자유로운 텍스트 입력을 받음           | 텍스트 입력란 | 의견, 설명, 개방형 응답이 필요한 경우     | "제품에 대한 의견을 자유롭게 작성해주세요."            |
|🔽 드롭다운   | 여러 옵션 중 하나를 선택하도록 함     | 드롭다운 목록 | 옵션이 많아 목록을 압축할 필요가 있을 때 | "거주 지역을 선택하세요. (서울, 부산, 대구, 광주…)"    |
이제 Markdown 지원되는 환경에서 표로 깔끔하게 보일 거예요. 😊
