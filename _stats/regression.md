...
`회귀분석(Regression Analysis)`은 두 개 이상의 변수 간의 관계를 모델링하는 기법입니다. 여기서 '회귀'는 `독립 변수`들이 `종속 변수`에 미치는 영향을 파악하는 과정입니다. 종속 변수는 <u>예측하고자 하는 대상</u>이며, 독립 변수는 <u>그 예측에 영향을 미칠 수 있는 변수</u>들입니다.
...
가장 기본적인 회귀분석으로, 독립 변수와 종속 변수 간의 선형 관계를 모델링합니다. `단순 선형 회귀`는 독립 변수가 하나일 때, `다중 선형 회귀`에서는 독립 변수가 여러 개일 때 사용됩니다.
